...
Most media servers are limited to a single CPU core. Mediasoup allows the application to spawn multiple workers, effectively scaling horizontally across all available hardware resources.

### 3. "Plain RTP" Support
Mediasoup supports **Plain RTP (Real-time Transport Protocol)**. This is the native language of the global telephony network (SIP/PSTN). This allows us to bridge calls from providers like **Vobiz or Exotel** directly into the engine without complex converters.

### 4. Zero Latency
Designed for sub-500ms communication. This is critical for AI Voice Bots to ensure the conversation feels natural and does not have awkward silences.
...
2. **Ingress:** The SIP Trunk sends the audio packets to the server via a **PlainRtpTransport**.
3. **Routing:** Mediasoup creates a **Router** to manage the call stream.
4. **AI Processing:** 
    - The audio is "Consumed" by a Python/Node.js worker.
    - Raw audio is sent to **Deepgram** (Speech-to-Text).
    - Text is sent to **Groq/Llama 3** (Brain).
    - Response audio is generated by **Cartesia/TTS**.
5. **Egress:** The AI audio is sent back to Mediasoup as a **Producer** and routed back to the customer's phone line.

---

## 💻 System Requirements
To implement a Mediasoup-based architecture, the following environment is required:

*   **Operating System:** Linux (Ubuntu 20.04+ recommended).
*   **Language Runtimes:** Node.js (v18+) or Rust.
*   **Build Tools:** `gcc`, `g++`, `make`, and `python3` (required to compile the C++ core during installation).
*   **Networking:** A server with a Public IP and a wide range of open UDP/TCP ports for media traffic.
...
| **Security** | Supports DTLS, SRTP, and encrypted media transport. |

**Final Recommendation:**
Mediasoup is the ideal choice for building a **proprietary, high-scale calling platform**. It provides the best performance-to-cost ratio for companies handling 20,000+ minutes of calls per month, though it requires a higher degree of technical implementation compared to higher-level frameworks.
...
- **Documentation:** [https://mediasoup.org/documentation/](https://mediasoup.org/documentation/)
